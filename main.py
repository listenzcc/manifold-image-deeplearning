...
def clip_manifold_video(image_dir, output="clip_manifold.mp4", interp_k=20, fps=24):
    ...
    print("向量插值...")
    z_interp = interpolate_clip(zs, k=interp_k)

    print("Diffusion 生成...")
    images = []
    for i, z in enumerate(z_interp):
        img = generator.generate(z, seed=42)
        images.append(img)
        print(f"  frame {i+1}/{len(z_interp)}")

    print("写入视频...")
    write_video(images, output, fps=fps)
...
if __name__ == "__main__":
    clip_manifold_video(
        ...
        interp_k=15,
        fps=24,
        output="clip_diffusion_manifold.mp4",
    )
